...
import { MapPin, Navigation, Camera, CheckCircle, Dog, Key, AlertTriangle, Loader2 } from "lucide-react";
...
const FieldApp = () => {
  ...
  const gateLabels: Record<string, string> = {
    side_gate: "Portail latéral",
    garage_code: "Code garage / porte",
    front_gate: "Portail avant",
  ...
  return (
    <div className="min-h-screen bg-background">
                  ...
                  <Card className={`shadow-card transition-all ${isDone ? "opacity-60" : "hover:shadow-card-hover hover:-translate-y-0.5"}`}>
                    <CardContent className="pt-4 pb-4">
                      <div className="flex items-start justify-between mb-2">
                        <div>
                          ...
                          <p className="text-xs text-muted-foreground flex items-center gap-1">
                            ...
                            {client?.address || "Adresse non définie"}
                          </p>
                        </div>
                      ...
                      <div className="flex items-center gap-3 text-xs text-muted-foreground mb-3">
                        ...
                        {dogDetails.length > 0 && (
                          <span>{dogDetails.map(d => d.name).join(", ")}</span>
                        )}
                      </div>
                      ...
                      {!isDone && (
                        <div className="flex gap-2">
                          ...
                          <Button
                            ...
                            onClick={() => setSelectedJob(intervention.id)}
                          >
                            Start Job 🚀
                          </Button>
                        </div>
      ...
      <Drawer open={!!selectedJob} onOpenChange={(open) => { if (!open) { setSelectedJob(null); setPhotoPreview(null); setPhotoFile(null); } }}>
        <DrawerContent className="max-h-[90vh]">
          <DrawerHeader>
            <DrawerTitle className="font-display">
              {selectedClient ? `${selectedClient.first_name} ${selectedClient.last_name}` : "Job"}
            </DrawerTitle>
